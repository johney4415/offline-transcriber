...
程式可以從你的校正中「學習」，全程本機比對、不使用網路：

**方式一（轉錄完直接改）**：在「轉錄」分頁的文字區直接修改辨識錯的字 →
按「**從修正學習**」→ 程式列出找到的修正，勾選後加入字典。

**方式二（事後比對檔案）**：在「**學習**」分頁選擇「原始轉錄檔」與「校正後檔案」→
按「開始比對」。記得儲存轉錄結果時保留一份未修改的原始檔。

學習的分類邏輯：

- 修正前後**發音相同**（汪曉明 → 王小明）→ 建議加入「詞彙表」（同音字修正）
- 發音相近（黎拉 → 李娜，l/n 不分）→ 建議加入詞彙表並標記「模糊音」
- **發音不同**（計畫 → 企劃）→ 建議加入「取代規則」
- 大段改寫、單純刪字 → 視為潤稿，不學習（避免學到一次性的修改）

所有建議都需要你勾選確認才會寫入字典，字典內容隨時可在「字典」分頁檢視與刪除。

## 重點整理（離線 AI 摘要）

「重點整理」分頁用本機語言模型（Qwen2.5-3B）整理逐字稿，**全程離線、不使用網路**：

1. 轉錄完成後，到「重點整理」分頁按「**帶入轉錄結果**」（或自行貼上任何文字）
2. 可自訂「整理指令」，例如：
   - `請整理這段逐字稿的重點，並條列出待辦事項與決議。`（預設）
   - `請抽出所有待辦事項，標註負責人與期限。`
   - `用三句話總結這場會議。`
3. 按「**整理重點**」，結果會即時逐字顯示，可儲存為 .txt

說明：

- 第一次整理需載入模型，約 10–30 秒；之後較快
- CPU 上整理一份會議稿約需 30 秒～3 分鐘（視長度與電腦效能）
- 超長逐字稿會自動分段整理再彙整
- 輸出一律轉為繁體中文（台灣用語）
- 小模型的摘要能力不及雲端服務，但條列重點、抓待辦堪用；忠於原文、不連網是它的取捨
...
- 重點整理：[llama-cpp-python](https://github.com/abetlen/llama-cpp-python) + Qwen2.5-3B-Instruct GGUF（CPU、長稿 map-reduce 分段、輸出 OpenCC 轉繁體）
